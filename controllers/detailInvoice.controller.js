...
const createDetailInvoice = async (req, res = response) => {
 ...
 const existingDetailInvoice = await DetailInvoice
  ...
  const distinctProducts = new Set(); // almacena productos distintos
  let totalInvoice = 0; // almacena el total de la factura


  detailInvoices.forEach(detailInvoice => {
    distinctProducts.add(detailInvoice.product.toString());
    totalInvoice += detailInvoice.precioTotal; // suma, precio total del producto a la factura
  });
  // verifica si el producto ha sido agregado anteriormente
  if (distinctProducts.has(product)) {
    return res.status(400).json({ msg: `The product ${product} has already been added to the invoice.` });
  }
  // agrega el productos al set de productos distintos
  distinctProducts.add(product);

  // verifica que no se han agregado más de 10 productos distintos
